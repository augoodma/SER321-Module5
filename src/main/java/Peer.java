...
public class Peer {
	...
	public static int nodesReceived = 0;
	public static boolean leader = false;
	public static boolean connected = false;
	public static List<String[]> nodes = new ArrayList<>();
	public Peer(BufferedReader bufReader, String username, ServerThread serverThread, String host, String port){
	...
	public static void main (String[] args) throws Exception {
		...
		else username = "follower";
		System.out.println("You are a  " + username + " node.");
		System.out.println("Your port number is " + args[2]);
		...
		serverThread.start();
		peer = new Peer(bufferedReader, username, serverThread, args[1], args[2]);
	...
	public void updateListenToPeers() throws Exception {
		if (leader) {
			...
			System.out.println("> Awaiting node data...");
		}
		askForInput();
	}
	...
	public static void askForInput() throws Exception {
		String message;
		try {
			if(!leader) System.out.println("Press enter to send your data to the leader.");
			...
			if (message.equals("exit")) {
				...
				serverThread.sendMessage("{'username': '" + host + "','message':'" + port + "'}");
				System.out.println("Data sent! Press enter to accept leader connection.");
			}
			if(connected && leader){
				message = bufferedReader.readLine();
				if (message.equals("exit")) {
					...
					for(String[] node : nodes)
						serverThread.sendMessage("{'username': '" + node[0] + "','message':'" + node[1] + "'}");
